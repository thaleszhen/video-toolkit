...
- 使用 `--config` 运行自定义工作流 JSON
- 使用 `--input-dir` / `--output-dir` 批量处理目录中的视频
- 运行单个处理模块（`trim`、`crop`、`compress`、`watermark`、`normalize`）
- 输入支持本地视频路径和 YouTube 链接
- 生成自定义工作流 JSON 模板，并可基于预设创建
...
- `--config <file>`：运行自定义工作流文件
- `--input-dir <dir>`：批量处理目录中的视频
- `--output-dir <dir>`：批量处理的输出目录
- `--dry-run`：只校验流程，不实际执行
...
- 本地文件输入：
  - 文件必须存在，否则会报错。
- 批量目录输入：
  - 仅处理当前目录下受支持的视频文件（如 `.mp4`、`.mov`、`.mkv`）。
- YouTube 链接输入：
  - 程序会先自动下载到临时目录，再继续处理。
  - 临时下载文件在命令结束后自动清理。
- 输出文件：
